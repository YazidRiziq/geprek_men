...
<section class="flex-1 bg-slate-50 dark:bg-slate-900/20 p-6">
    ...
    <div class="max-w-3xl mx-auto bg-white dark:bg-background-dark rounded-xl border border-slate-200 dark:border-primary/10 shadow-sm">
        ...
        <!-- Header -->
        <div class="px-6 py-4 border-b border-slate-200 dark:border-primary/10 flex items-center justify-between">
            <div>
        ...
        <!-- Form -->
        <form action="<?= base_url('admin/categories/update/' . $category['id']) ?>" method="post" class="p-6 space-y-6">
            <?= csrf_field() ?>
            ...
            <div>
                ...
                <input
                    type="text"
                    name="cat_name"
                    value="<?= esc($category['cat_name']) ?>"
                    required
                    class="w-full rounded-lg border border-slate-300 dark:border-primary/20
                           bg-white dark:bg-background-dark
                           px-4 py-2 text-slate-900 dark:text-slate-100
            ...
            <div>
                ...
                <textarea
                    ...
                    class="w-full rounded-lg border border-slate-300 dark:border-primary/20
                           bg-white dark:bg-background-dark
                           px-4 py-2 text-slate-900 dark:text-slate-100
                           focus:outline-none focus:ring-2 focus:ring-primary/50"><?= esc($category['description']) ?></textarea>
            </div>
